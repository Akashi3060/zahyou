...
def build(out_path):
    ...
    ext(steps([
        ...
        "ブラウザが「一般的にダウンロードされていません」と言ったら、"
        "<b>［保存］</b>を選びます（2.2）。",
        "ダブルクリックします。初回の起動に 4 秒ほどかかります。",
        "青い画面が出たら、<b>「詳細情報」→「実行」</b>を押します（2.2）。",
    ], st))
    ...
    add(Paragraph("zahyou には<b>開発元の署名（コード署名証明書）が付いていません</b>。"
                  ...
                  "付いていないことがよくあります。"
                  "そのため、落としてから動かすまでに<b>最大 3 回</b>止められます。"
                  "順に見ていきます。<b>作った本人の PC では 1 つも出ません</b>ので、"
                  "配る側は気づきにくい種類の警告です。", st.body))

    add(Paragraph("① ブラウザ「一般的にダウンロードされていません」", st.h3))
    ext(steps([
        "ダウンロード一覧に出た <b>zahyou.exe</b> の行で "
        "<b>［…］</b>（その他の操作）を押します。",
        "<b>［保存］</b>を選びます。",
    ], st))
    ext(callout("［保存］を押すまで、exe は出来上がりません", [
        "押さずに放っておくと、フォルダーには "
        "<b>「未確認 841468.crdownload」</b>のような名前の"
        "書きかけのファイルが残ったままになります。"
        "<b>zahyou.exe が見当たらないときは、たいていこれです。</b>",
        "Chrome も Edge も、「まだ広く出回っていないファイル」というだけで"
        "この警告を出します。中身を見て危ないと判断したわけではありません。",
    ], st))

    add(Paragraph("② SmartScreen「Windows によって PC が保護されました」", st.h3))
    add(Paragraph("青い画面が出て、「実行」ボタンが見当たりません。"
                  "<b>「詳細情報」をクリックすると「実行」ボタンが現れます。</b>",
                  st.body))
    add(Paragraph("ダウンロードしたファイルには「別の PC から来た」という印が付きます。"
                  "署名が無いと、Windows はこの印を見てここで一度止めます。",
                  st.small))

    add(Paragraph("③ スマート アプリ コントロール（灰色の画面）", st.h3))
    add(Paragraph("<b>これだけは「実行」で通せません。</b>"
                  "「スマート アプリ コントロールが安全でない可能性のある"
                  "アプリをブロックしました」と出て、ボタンが"
                  "［OK］と［Microsoft Store からアプリを入手してください］"
                  "しかありません。署名の無いアプリを一律に止める仕組みなので、"
                  "その場で許可する方法が用意されていません。", st.body))
    add(Paragraph("Windows 11 を新しく入れた PC で有効になっていることがあります。"
                  "次の<b>どちらか</b>を選んでください。", st.body))
    ext(table([
        ["", "やり方", "引きかえに"],
        ["A. ノートブック版を使う",
