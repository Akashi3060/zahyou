...
def build(out_path):
    ...
    ext(h1("準備するもの", st, "3"))
    ext(table([
        ["項目", "これは要ります", "これくらいあれば安心"],
        ["パソコン", "Windows 10 / 11（64bit）",
         "同じ（オフライン解析には WSL2 が動くこと）"],
        ["CPU", "2 コア",
         "<b>コア数より 1 コアの速さが効きます</b>（下記）"],
        ["メモリ", "オンラインのみ：2 GB<br/>オフラインも：<b>4 GB</b>",
         "<b>8 GB</b>（一眼レフの 24 メガ画素なら 16 GB）"],
        ["ディスクの空き", "オンラインのみ：0.2 GB<br/>オフラインも：<b>4 GB</b>",
         "<b>16 GB</b> の SSD（下の内訳）"],
        ["インターネット", "導入のときだけ",
         "運用時はオフラインで構いません"],
        ["天体画像", "FITS / FIT / JPEG / PNG / TIFF",
         "星が 10 個ほど写っていれば十分です"],
    ], st, widths=[24 * mm, 46 * mm, CONTENT_W - 70 * mm]))

    ext(h2("どれくらい使うか（実測）", st))
    add(Paragraph("Core Ultra 7 265K・メモリ 191 GB の PC で測った値です。"
                  "画像 1 枚を解くあいだの<b>いちばん多いとき</b>を載せています。",
                  st.small))
    ext(table([
        ["画像の大きさ", "メモリ", "星を探す時間"],
        ["968 x 548（切り出した動画の 1 コマ）", "280 MB", "0.2 秒"],
        ["1936 x 1096（ASI290MM 全画面）", "約 330 MB", "0.8 秒"],
        ["4144 x 2822（ASI2600MC 全画面）", "約 690 MB", "4.3 秒"],
        ["6000 x 4000（一眼レフ 24 メガ画素）", "約 1.2 GB", "8.9 秒"],
    ], st, widths=[74 * mm, 30 * mm, CONTENT_W - 104 * mm],
        align=[None, "c", "c"]))
    add(Paragraph("だいたい <b>1 メガ画素あたり 40 MB</b> です。"
                  "オフライン解析のときは、これに WSL のぶん（1 GB 前後）が"
                  "加わります。solve-field 自体は 130 MB ほどしか使いません。",
                  st.body))

    ext(callout("CPU はコア数を増やしても速くなりません", [
        "同じ 24 メガ画素の画像で、使えるコア数を変えて測りました。"
        "<b>20 コア 8.9 秒 / 4 コア 9.2 秒 / 2 コア 9.3 秒 / 1 コア 10.2 秒</b>。"
        "星を探す処理はほとんど 1 コアしか使いません。",
        "星の並びを照合する solve-field も、"
        "<b>1 コア 2.8 秒 / 20 コア 3.4 秒</b>とほぼ変わりません。"
        "こちらは計算ではなく<b>星図データの読み込み待ち</b>で時間が決まるためです。",
        "つまり、効くのは<b>1 コアあたりの速さ</b>と"
        "<b>星図データを置くディスクの速さ</b>です。"
        "コア数の多い CPU にお金をかけるより、SSD に置くほうが効きます。",
    ], st, kind="note"))
    ...
    ext(table([
        ...
        ["星図データ 基本セット（11′〜2000′）", "1.3 GB", "オフライン解析のとき"],
        ["＋ 4′ まで（5204 / 5203 / 5202）", "計 9.1 GB", "長焦点・切り出しのとき"],
        ["＋ 2′ まで（5201 / 5200）", "計 34 GB", "さらに狭いとき"],
        ["解析中の一時ファイル", "画素数 x 4 バイト", "解析のあいだだけ"],
    ], st, widths=[74 * mm, 26 * mm, CONTENT_W - 100 * mm],
        align=[None, "c", None]))
    add(Paragraph("一時ファイルは 24 メガ画素で 96 MB ほどです。"
                  "終われば自動で消します。", st.small))
