...
def build(out_path):
    ...
    ext(table([
        ...
        ["画面", "3 つの短いセルだけ。全部で 90 行ほどに収まります"],
    ...
    ext(callout("オンラインだけで使うなら、3.4 以降は飛ばせます", [
        "インターネットにつながる場所でしか使わないのであれば、"
        "WSL と星図データ（合計 15 GB ほど）は要りません。"
        "3.1 〜 3.3 と 3.8 だけ済ませてください。",
        "あとから追加することもできます。",
    ], st, kind="note"))
    ...
    ext(h2("【オフラインで使う人だけ】WSL を入れる", st, "3.4"))
    ...
    ext(callout("管理者のターミナルでないと失敗します", [
        ...
        "オンライン解析のみでお使いください（3.1〜3.3 と 3.8 だけで動きます）。",
    ], st))

    ext(h2("【同】解析エンジン astrometry.net を入れる", st, "3.5"))
    add(Paragraph("ここから先は、<b>zahyou に同梱の setup_wsl.ps1 を使うと"
                  "3.5 〜 3.7 をまとめて片付けられます</b>。"
    ...
    ext(callout("まず何を落とせばよいか迷ったら", [
        ...
        "それより長い焦点距離を使う場合は、5 章の表を見て "
        "<b>index-5204 / 5203 / 5202</b> を必要な分だけ足してください。",
    ], st, kind="note"))

    ext(h2("【同】設定ファイルを書く", st, "3.7"))
    ...
    add(Paragraph("下の表は ZWO ASI290MM（横 1936 画素・画素 2.9 µm）を例にした"
                  "目安です。ビニングしても画角は変わりません。", st.small))
    ext(table([
    ...
    add(Paragraph("上の 3 段（4107〜4119 / 5206 / 5205）が基本セット "
                  ...
                  "下の 5 つから必要なものを足してください。", st.body))
    ext(callout("★ 掩蔽観測で画面を切り出している人は「短いほうの辺」で選ぶ", [
        "ローリングシャッターの影響を抑えたり、コマ落ちを防ぐために、"
        "読み出す範囲（特に<b>縦</b>）を切り詰めて撮ることがあります。"
        "このとき横幅は変わらないのに、<b>短辺の画角だけが極端に狭くなります</b>。",
        "星の並び（四角形）は<b>短いほうの辺に収まる大きさまでしか作れません</b>。"
        "つまり必要な星図データは短辺で決まります。"
        "横幅の画角だけを見て選ぶと、足りずに解けません。",
        "例: 焦点距離 800 mm・ASI290MM を 1936 x 400 画素に切り出すと、"
        "横は 24′ でも縦は <b>5.0′</b>。横だけ見ると index-4107（22′〜30′）で"
        "足りるように見えますが、実際には <b>index-5202（4′〜5.6′）</b>まで要ります。",
        "デスクトップ版の［準備］タブは、画像を選んでおくと"
        "<b>切り出した縦横を読んで短辺のぶんまで選びます</b>。"
        "ノートブック版でも、解けなかったときは短辺の画角と"
        "必要な段を画面に出します。",
    ], st))
    ...
    ext(table([
        ...
        ["解析に失敗する（星が少ない）",
         ...
         "撮影条件を見直してください。"],
        ["解析に失敗する（画角が合わない）",
         "5 章の表を見て、自分の画角に合う星図データを追加してください。"
         "失敗時のメッセージに必要なファイル名が出ます。"],
        ["オフラインで「WSL が使えません」",
         "3.4 をやり直してください。管理者のターミナルでないと入りません。"],
        ["オフラインで「solve-field がありません」",
         "3.5 の apt install が終わっていません。"],
        ["オフラインで「星図データが見つかりません」",
         "3.6 の展開先か、3.7 の add_path が間違っています。"
         "C:\\AstrometryData の直下に index-....fits がある状態にしてください。"],
        ["オフライン解析がとても遅い",
         "3.7 の「速くしたい人へ」を実施してください。"
