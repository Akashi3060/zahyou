...
|---|---|---|
| パソコン | Windows 10 / 11（64bit） | 同じ（オフライン解析には WSL2） |
| CPU | 2 コア | **コア数より 1 コアの速さ**（下記） |
| メモリ | オンラインのみ 2 GB ／ オフラインも **4 GB** | **8 GB**（24 メガ画素なら 16 GB） |
| ディスクの空き | オンラインのみ 0.2 GB ／ オフラインも **4 GB** | **16 GB** の SSD |
| インターネット | 導入のときだけ | 運用時はオフラインで構いません |
| 天体画像 | 星が 10 個ほど写っていれば十分 | |

<details>
<summary><b>どれくらい使うか（実測値）</b></summary>

Core Ultra 7 265K で、画像 1 枚を解くあいだのいちばん多いときを測った値です。

| 画像の大きさ | メモリ | 星を探す時間 |
|---|---|---|
| 968 × 548（切り出した動画の 1 コマ） | 280 MB | 0.2 秒 |
| 1936 × 1096（ASI290MM 全画面） | 約 330 MB | 0.8 秒 |
| 4144 × 2822（ASI2600MC 全画面） | 約 690 MB | 4.3 秒 |
| 6000 × 4000（一眼レフ 24 メガ画素） | 約 1.2 GB | 8.9 秒 |

だいたい **1 メガ画素あたり 40 MB**。オフラインではこれに WSL のぶん
（1 GB 前後）が加わります。solve-field 自体は 130 MB ほどです。

**CPU はコア数を増やしても速くなりません。**
同じ 24 メガ画素で、20 コア 8.9 秒 / 4 コア 9.2 秒 / 2 コア 9.3 秒 / 1 コア 10.2 秒。
solve-field も 1 コア 2.8 秒 / 20 コア 3.4 秒 とほぼ同じで、
こちらは星図データの**読み込み待ち**で時間が決まります。
効くのは **1 コアの速さ**と**星図データを置くディスクの速さ**です。
...
|---|---|
| `zahyou.exe` | 102 MB |
| WSL（Ubuntu ＋ astrometry.net） | 2.4 GB |
| 星図データ 基本セット（11′〜2000′） | 1.3 GB |
| ＋ 4′ まで（5204 / 5203 / 5202） | 計 9.1 GB |
| ＋ 2′ まで（5201 / 5200） | 計 34 GB |
| 解析中の一時ファイル | 画素数 × 4 バイト（24 メガ画素で 96 MB） |
