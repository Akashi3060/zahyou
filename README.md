...
詳しい説明は
**[導入・運用マニュアル v3（PDF・32 ページ）](天体画像解析プログラム「zahyou」導入・運用マニュアルv3.pdf)**
にあります。
...
<img src="docs/img/dl-1-save.png" width="560" alt="ダウンロード欄に「zahyou.exe で行う操作を選んでください。」と出て、開く / 名前を付けて保存 / 保存 の 3 つのボタンが並んでいる。右端の「保存」が赤枠で示されている">

### 2. 「一般的にダウンロードされていません」→ ［…］→ ［保存］

<img src="docs/img/dl-2-keep.png" width="620" alt="「zahyou.exe は一般的にダウンロードされていません」という盾のアイコン付きの警告。行の右端の […] ボタンと、開いたメニューの中の「保存」が赤枠で示されている">

ゴミ箱の右にある **［…］**（その他の操作）を押すと、メニューに **［保存］** が出ます。

### 3. 「開く前に、信頼できることを確認してください」→ ［削除］の ▽ → ［保持する］

<img src="docs/img/dl-3-trust.png" width="480" alt="「zahyou.exe を開く前に、信頼できることを確認してください」というダイアログ。右下の「削除」ボタンの下向き矢印と、そこから出た「保持する」が赤枠で示されている">
...
<img src="docs/img/dl-4-open.png" width="560" alt="ダウンロード欄に zahyou.exe と「ファイルを開く」のリンクが出ている。リンクが赤枠で示されている">

### 5. 青い「Windows によって PC が保護されました」→「詳細情報」→「実行」

<img src="docs/img/run-1-smartscreen.png" width="620" alt="青い全画面の SmartScreen 警告の再現図。「詳細情報」のリンクと、それを押すと現れる「実行」ボタンが赤枠で示されている">
...
<img src="docs/img/run-2-smart-app-control.png" width="620" alt="「スマート アプリ コントロールが安全でない可能性のあるアプリをブロックしました」という灰色のダイアログの再現図。ボタンは OK と Microsoft Store からアプリを入手してください の 2 つだけ">
...
1 つの exe に Python のプログラムをまとめているため、誤検知されることがあります。
その場合はウイルス対策ソフト側で `zahyou.exe` を除外に登録してください。

中身を確かめたい方は、[Releases](https://github.com/Akashi3060/zahyou/releases/latest)
に載せている SHA256 と、手元のファイルのハッシュを見比べてください。
...
<img src="docs/img/app-analyze.png" width="860" alt="zahyou の解析タブ。上に画像のパスと「解析を実行」、左に目標・望遠鏡・解析の設定、右上に 3 つの大きな数字、その下に星が緑の丸で囲まれた画像と、目標を指す黄色い丸と赤い矢印">

ふだん触るのは**左の列**と**「解析を実行」**だけです。

1. 「参照...」で画像を選ぶ（FITS / FIT / JPEG / PNG / TIFF）
2. 目標を入れる（天体名か、赤経・赤緯）
3. 「解析を実行」

### 出た数字の読み方

<img src="docs/img/app-numbers.png" width="700" alt="画像中心から目標までの距離 1.583′、赤経 (RA) 方向 0.918′ 東、赤緯 (Dec) 方向 1.290′ 北。その下に画像中心の座標・画素スケール・視野・所要時間">

この**3 つが答え**です。いま向いている点（画像の中心）から目標まで、
**赤経方向に 0.918′ 東へ、赤緯方向に 1.290′ 北へ**動かせばよい、という意味です。
...
<img src="docs/img/app-northup.png" width="620" alt="「北が上」タブの図。北を上・東を左にそろえ直した画像に、水色の × と黄色の ○、赤い矢印が描かれている">

水色の **×** が画像の中心、黄色の **○** が目標、**赤い矢印**が動かす向きです。

図の下のボタンで、拡大して星の位置を確かめたり、PNG で保存したりできます。

<img src="docs/img/app-toolbar.png" width="560" alt="図の下のボタン。全体・戻る・進む・移動・拡大・画像を保存">

---

## ネットの無いところで使う — ［準備］タブ

<img src="docs/img/app-prepare.png" width="860" alt="zahyou の準備タブ。WSL・astrometry.net・星図データ・設定ファイルの状態が一覧表で OK と表示され、その下に「まとめて準備する」ボタン、星図データの枠に「おすすめをまとめて落とす」ボタンと段ごとのチェック欄">

**「まとめて準備する」を押すだけ**です。WSL・解析エンジン・星図データ・設定ファイルを
順に用意します（初回のみ・数十分）。
一覧表の「総合」が**「オフライン解析できます」**になれば完了です。

### 星図データはボタン 2 つ

<img src="docs/img/app-index.png" width="420" alt="星図データの枠。置き場所の入力欄、「おすすめをまとめて落とす (約 9.5 GB)」ボタン、焦点距離とセンサー横幅の入力欄と「必要な段を選ぶ」ボタン、index-5200 から 5206 までのチェック欄">
...
<img src="docs/img/app-dark.png" width="860" alt="暗い配色にした解析タブ。背景・入力欄・図の中まで黒基調になっている">
...
<img src="docs/img/app-target.png" width="300" alt="目標の枠。天体名で指定 / 赤経・赤緯で指定 / 指定しない (完全ブラインド) の 3 つから選ぶ">

上の 3 つの数字は、**画像中心の赤経・赤緯・画素スケール**に変わります。

<img src="docs/img/app-blind-numbers.png" width="700" alt="完全ブラインドのときの 3 つの数字。画像中心 (赤経) 4h07m33.9s、画像中心 (赤緯) +41d57m53s、画素スケール 1.462″/px">
...
<img src="docs/img/app-help.png" width="860" alt="zahyou の使い方タブ。上に更新の確認と更新ボタン、下に使い方・出た数字の読み方・ネットが無いところで使う などの説明が並んでいる">

操作の要点と、設定の置き場所がまとまっています。
**新しい版への入れ替えも、上の「更新を確認」→「更新する」**でできます。

### そのほか

- **オフライン解析のときは**、**黒い画面が一瞬出ては消える**ことがあります
  （Windows の中の Linux に計算を頼んでいる合図で、故障ではありません）

| | |
|---|---|
| 大きさ | 102 MB（ファイル 1 個） |
| 動作環境 | Windows 10 / 11（**x64**）／ メモリ 4 GB から（[下記](#動作環境)） |
| オフライン解析 | ［準備］タブから自動で用意（WSL + 星図データ 4 GB 〜） |

中で何をしているかは **[gui/README.md](gui/README.md)** に書いてあります。

---

# 動作環境

デスクトップ版・ノートブック版に共通です。
重さは **画像の画素数**と**星図データの量**で決まります。
オンライン解析だけなら、ごく軽い PC でも動きます。

| | 最小 | 推奨 | オフラインも使う |
|---|---|---|---|
| | オンライン / 〜6 メガ画素 | オンライン / 〜24 メガ画素 | WSL2 ＋ 星図データ |
| CPU | x64 2 コア 2.0 GHz | **4 コア 8 スレッド** | 同左（仮想化を有効に） |
| メモリ | **4 GB** | **8 GB** | 8 GB（16 GB あると速い） |
| ディスク | 空き 1 GB | 空き 2 GB・SSD | 空き 5 GB（4′ まで入れるなら 20 GB） |
| GPU | 不要（内蔵で十分） | | |
| 画面 | 1280 × 800 以上 | | |
| ネット | 常時 | 常時 | 初回の用意のときだけ |

> **ARM 版の Windows では使えません。**
> `zahyou.exe` は x64 専用に作ってあります。Snapdragon X などの ARM 機では
> 変換されながら動くため、計算が主なこのプログラムでは実用的な速さになりません。

<details>
<summary><b>実測値と、その理由</b></summary>

### メモリは画素数でほぼ決まる

実際の `zahyou.exe` で、画像 1 枚を最後まで解いたときの**いちばん多いとき**です。

| 画像の大きさ | メモリ | オフライン解析 |
|---|---|---|
| 968 × 548（0.5 メガ画素・切り出した動画） | 227 MB | 6.4 秒 |
| 2904 × 1644（4.8 メガ画素） | 834 MB | 16.5 秒 |
| 5808 × 3288（19.1 メガ画素） | 2.8 GB | 42.7 秒 |

きれいに直線に乗ります —— **おおよそ 140 MB × メガ画素 ＋ 160 MB**。
一眼レフの 24 メガ画素なら 3.5 GB、48 メガ画素なら 7 GB 前後です。
メモリを使うのは星を探す処理ではなく、**「北が上」の画像を作り直す処理**です。

この数字は、作り直しを小分けにして減らしたあとのものです。
そのまま走らせると 19.1 メガ画素で 2,581 MB 要りましたが、
小分けにすると **557 MB** で済み、結果は 1 画素も変わりませんでした（時間は 2 割増）。
アプリ全体でも **5.9 GB → 2.8 GB** に下がっています。

### CPU はコア数を増やしても速くなりません
...
前者はほとんど 1 コアしか使いません。
後者は計算ではなく**星図データの読み込み待ち**で決まります
（CPU を使っている時間は全体の 3 割ほど）。

それでも推奨を **4 コア 8 スレッド**としているのは、解析が速くなるからではなく、
**Windows・WSL・画面がそれぞれ CPU を使うから**です。
2 コアでも解析はできますが、解析中はほかの操作がもたつきます
（ノート PC での実測にもとづきます）。
つまり効くのは **1 コアあたりの速さ**と**星図データを置くディスクの速さ**で、
コア数ではありません。
...
ハードウェアの目安は上の **[動作環境](#動作環境)** をご覧ください。
そのほかに要るものは次のとおりです。
...
> インターネットにつながる場所でしか使わないなら、WSL と星図データ（4 GB ほど、細かい段まで入れるなら 12 GB ほど）は要りません。
> あとから追加することもできます。
...
落としたものは入れ替える前に **SHA256 で中身を確かめ**、違えば捨てます。
いままでの版は `zahyou.exe.bak` として横に残ります。

起動したときにも自動で確認します（新しい版があるときだけ知らせます）。
確認をやめたいときは、同じところのチェックを外してください。

手で入れ替えるなら、[Releases](https://github.com/Akashi3060/zahyou/releases/latest)
