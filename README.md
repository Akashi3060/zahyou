...
> ### ⚠ 落としてから動かすまでに、最大 3 回止められます
>
> **zahyou に開発元の署名（コード署名証明書）が付いていない**ためです。
> 証明書は毎年お金がかかるもので、個人が配るソフトには付いていないことが
> よくあります。**作った本人の PC では 1 つも出ません。**
>
> **① ブラウザ「一般的にダウンロードされていません」**
> ダウンロード一覧の `zahyou.exe` の行で **［…］→［保存］**。
> これを押すまで exe は出来上がらず、`未確認 841468.crdownload` のような
> 書きかけのファイルが残ります。**exe が見当たらないときは、たいていこれです。**
>
> **② SmartScreen「Windows によって PC が保護されました」（青）**
> **「詳細情報」をクリック → 現れた「実行」ボタンを押す**。
>
> **③ スマート アプリ コントロール（灰色）**
> 「安全でない可能性のあるアプリをブロックしました」と出て、
> **この画面では通せません**（［OK］しかありません）。
> 署名の無いアプリを一律に止める仕組みなので、その場で許可できません。
...
> 入れ直さずに戻せることもあります）。ここは PC を使う本人の判断です。
> 決めかねるときは A を選んでください。
>
> ウイルス対策ソフトが誤検知することもあります（1 つの exe にまとめているため）。
> 中身が本物か確かめたい方は、[Releases](https://github.com/Akashi3060/zahyou/releases/latest)
...
星図データは **「おすすめをまとめて落とす」**（画角 4′ 以上・約 9.5 GB）なら
焦点距離を入れずに押せます。
