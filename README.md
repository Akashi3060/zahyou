...
**［解析］タブ** — ふだん触るのは左の列と「解析を実行」だけです。

<img src="docs/img/app-1-analyze.png" width="820" alt="zahyou の解析タブ。左に目標・望遠鏡・解析の設定、右上に「画像中心から目標までの距離 1.583′」「赤経 (RA) 方向 0.918′ 東」「赤緯 (Dec) 方向 1.290′ 北」の 3 つの数字、その下に星が緑の丸で囲まれた画像と、目標を指す黄色い丸と赤い矢印">

上の**3 つの数字**が答えです。水色の × が画像の中心、黄色の ○ が目標、
赤い矢印が動かす向きです。

**［準備］タブ** — ネットの無いところで使うための用意をします。

<img src="docs/img/app-2-prepare.png" width="820" alt="zahyou の準備タブ。WSL・astrometry.net・星図データ・設定ファイルの状態が一覧表で OK と表示され、その下に「まとめて準備する」ボタン、星図データの枠に「おすすめをまとめて落とす (約 9.5 GB)」ボタンと段ごとのチェック欄">

**「まとめて準備する」を押すだけ**です。星図データは
**「おすすめをまとめて落とす」**（画角 4′ 以上・約 9.5 GB）なら
**焦点距離もセンサー横幅も入れずに**押せます。

---

### そのほか

- **暗いところで使うときは** … 右下の「🌙 暗く」。画面もタイトルバーも図の中も黒基調に
- **目標が分からないときは** … 「指定しない (完全ブラインド)」で、
  「この画像はどこを向いているか」だけを求められます
